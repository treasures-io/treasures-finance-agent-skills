...
- **Not stable over a cell's lifetime.** A `low_volume` warning flips to `settlement_failure` once a real order dies on that cell. Cache it no longer than the endpoint's 5-min server TTL; don't hold it past that.
