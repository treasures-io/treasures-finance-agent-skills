...
> ⚠️ **Base (`chain:"base"`) holdings are NOT on `/portfolio` — and this is not a `partial` case.** B20 positions and your Base USDC cash are absent from `positions`, from `usdc` (which carries `sol`/`eth` only), and from `usdg` — the response is a normal `200` with no flag saying anything is missing, so a wallet holding only B20 reads as empty here. Read Base balances from your own 8453 RPC (`balanceOf` on the `coinbase` block's `address`, and on Base-native USDC `0x8335…2913`). This affects **valuation only**: Base *trades* are first-class in `/trades` and `/settlements`, and a Base **sell** is still sized server-side against your live 8453 balance. See [`trading.md`](trading.md#base).
