...
5. **Not on `/portfolio`, and no reconciler — you MUST submit to get a `/trades` row.** Unlike sol/eth (and unlike Robinhood, whose positions read live), `/portfolio` does **not** report B20 positions or your Base USDC cash at all. Read those from your own 8453 RPC. A Base trade you never submit is absent from `/trades` and `/settlements` with no cost basis. Submitted trades do get on-chain-observed cost basis + P&L and appear in both.
