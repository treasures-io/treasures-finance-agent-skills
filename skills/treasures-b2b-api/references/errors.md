...
- **`tradfi_reference` is always present on a 200.** It used to be nullable on a tradfi-feed outage; a quote that cannot be price-checked against the reference is now refused with 422 `no_routes` / `reason: "reference_unavailable"` instead of returned unchecked, so a successful quote always carries one.
