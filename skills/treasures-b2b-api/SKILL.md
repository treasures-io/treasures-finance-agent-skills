...
- **`chain` takes one value or an array.** One chain pins; `["sol","eth"]` auto-routes within that set only (unique, at most one leg per member); `null`/omitted routes across every chain the ticker lists. An empty intersection → `422 no_routes`.
- **Check `tradability`/`warn_reason` before submitting.** Quote legs, `/stocks/tickers` and `/stocks` carry them; a leg can price normally and still be warned (`settlement_failure` = quotes fine, settles never). Action table in [`references/trading.md`](references/trading.md#tradability).
...
```ts
const SKILL_VERSION = '1.8.0'; // = SKILL.md metadata.version
